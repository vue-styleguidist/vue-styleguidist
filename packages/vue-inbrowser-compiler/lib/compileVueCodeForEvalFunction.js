...
export default function compileVueCodeForEvalFunction(code, config) {
  let style, vsgMode, template; // if the component is written as a Vue sfc,
  ...
  if (!/new Vue\(/.test(code)) {
    const findStartTemplateMatch = /^\W*</.test(code) ? {
      index: 0
    } : code.match(/\n[\t ]*</);
  ...
  walkes(ast, {
    ...
    ExpressionStatement(node) {
      if (node.expression.type === 'NewExpression' && node.expression.callee.name === 'Vue') {
        ...
        const optionsNode = node.expression.arguments && node.expression.arguments.length ? node.expression.arguments[0] : undefined;
  ...
  if (vsgMode) {
    code += `;return {data:function(){return {${// add local vars in data
