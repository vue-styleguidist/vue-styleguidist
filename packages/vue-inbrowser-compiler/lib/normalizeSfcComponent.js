...
function getSingleFileComponentParts(code) {
  ...
  if (parts.script) parts.script.content = parts.script.content.replace(/\/\*[\s\S]*?\*\/|([^:]|^)\/\/.*$/gm, '$1');
  return parts;
}
...
export default function normalizeSfcComponent(code) {
  ...
  const extractedComponent = injectTemplateAndParseExport(parts); //console.log(extractedComponent.component)

  return {
    component: [extractedComponent.preprocessing, `new Vue(${extractedComponent.component});`, extractedComponent.postprocessing].join('\n'),
