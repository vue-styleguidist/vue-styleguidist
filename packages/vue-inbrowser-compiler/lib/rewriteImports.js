...
function generate(keys, dep, base, fn) {
  const tmp = dep.split('/').pop().replace(/\W/g, '_') + '$' + num++; // uniqueness
...
export default function (str, fn = 'require') {
  num = 0;
  return str.replace(NAMED, (_, asterisk, base, req, dep) => generate(req ? req.split(',').filter(dep => dep.trim()) : [], dep, base, fn)).replace(UNNAMED, (_, dep) => `${fn}('${dep}');`);
}